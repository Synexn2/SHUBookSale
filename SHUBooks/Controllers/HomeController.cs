...
namespace SHUBooks.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            //service.BookFilter();
            ViewBag.TopTenRate = service.GetBooks().Take(10);
        ...
        public ActionResult ViewUserBook(int BookId, int rate = 0)
        {
            ViewBag.Rated = "False";
            ViewBag.RateEnable = rate;
            if(User.Identity.GetUserId() != null)
            {

                string login = MyUserManager.FindById(User.Identity.GetUserId()).Id;
                if (login != null || login != "")
                {

                    bool Rated = service.CheckIfUserRated(BookId, login);
                    if (Rated)
                        ViewBag.Rated = "True";
                }
            }
        ...
        [HttpPost]
        public ActionResult ViewUserBook(int BookId,string Rating)
        {
